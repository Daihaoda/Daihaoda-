...
# 3. 构造训练数据集
TIME_STEP = 5
def create_dataset(tsla_data, spy_data, vix_data, volume_data, time_step):
    x, y = [], []
    for i in range(len(tsla_data) - time_step):
        # 合并 TSLA, SPY, VIX 和 Volume 数据，形成多特征输入
        combined_features = np.column_stack((tsla_data[i:(i + time_step)],
                                             spy_data[i:(i + time_step)],
                                             vix_data[i:(i + time_step)],
                                             volume_data[i:(i + time_step)]))
        x.append(combined_features)  # 每个时间步的数据有四个特征
        y.append(tsla_data[i + time_step])  # 预测目标是 time_step 后的 TSLA 价格
    return np.array(x), np.array(y)

# 重新生成数据集
vix_change =  np.concatenate((np.array([0.0]),(prices_vix[1:]- prices_vix[:-1]) / prices_vix[:-1]))
volume_tsla_change = np.concatenate((np.array([0.0]), (volume_tsla[1:]- volume_tsla[:-1]) / volume_tsla[:-1]))

x_np, y_np = create_dataset(prices_tsla, prices_spy, vix_change, volume_tsla_change, TIME_STEP)
